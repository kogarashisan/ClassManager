...
/*ES5>*/
var ERROR_DESCRIPTIONS = require("./ERROR_DESCRIPTIONS");
/*<ES5*/

/*ES6>
import ERROR_DESCRIPTIONS from "./ERROR_DESCRIPTIONS";
<ES6*/
...
function _t(version, args, error_descriptions, url, prefix) {
	...
	if (typeof(message) == 'number') {
		/*__BUILD_COMMENT__**__CM_INCLUDE_ERROR_DESCRIPTIONS__**istanbul ignore else*/
...
function t(message) {
	...
	_t(
		...
		/* istanbul ignore next */ __CM_INCLUDE_ERROR_DESCRIPTIONS__ ? ERROR_DESCRIPTIONS : null,
		__CM_ERROR_DECODER_URL__,
		"[ClassManager error] "
	);
...
function isInlineArray(items) {
	...
	if (count > __CM_MAX_INLINED_ARRAY_LENGTH__) {
	...
	for (name in class_body) {
		...
		switch (type) {
			...
			case TYPES.ARRAY:
				...
				} else if (__CM_INLINE_SIMPLE_ARRAYS__ && isInlineArray(value)) {
				...
				break;
			case TYPES.UNDEFINED:
				__CM_ENABLE_SAFE_MODE__ && t(13, name);
				break;
			default:
				__CM_ENABLE_SAFE_MODE__ && t(14, type);
				break;
		}
...
function buildRealConstructor(class_data) {
	...
	if (class_data.skeleton._afterInit) {
		...
		if (__CM_ENABLE_SAFE_MODE__ && class_data.skeleton._afterInit.t != MEMBER_TYPES.FUNCTION) t(24);
		source += "\nthis._afterInit();";
...
function extendClass(child_data, child_skeleton, parent_data, parent_skeleton, is_root, references_offset) {
	...
	for (name in parent_skeleton) {
		...
		if (name in child_skeleton) {
			...
			if (__CM_ENABLE_SAFE_MODE__ && is_root && (child_skeleton[name].t == MEMBER_TYPES.FUNCTION ^ parent_type == MEMBER_TYPES.FUNCTION)) {
			...
			if (parent_type == MEMBER_TYPES.FUNCTION) {
				...
				if (__CM_ENABLE_SAFE_MODE__ && (new_name in child_skeleton)) t(12, new_name);
...
ClassManager = {
	...
	define: function(class_name, class_body) {
		...
		if (__CM_ENABLE_SAFE_MODE__ && !ALLOWED_CLASS_NAME_REGEX.test(class_name)) t(27, class_name);
		...
		if ('Class' in class_body) {
			...
			if (__CM_ENABLE_SAFE_MODE__ && !class_options) t(9, 'Class');
			if (class_options.is_abstract) class_data.is_abstract = true;
		}

		if ('Prepare' in class_body) {
			if (__CM_ENABLE_SAFE_MODE__ && typeof class_body.Prepare != 'function') t(9, 'Prepare');
		...
		if ('Extends' in class_body) {
			...
			if (__CM_ENABLE_SAFE_MODE__ && !this.isValidClass(_extends)) t(9, 'Extends');
			parent_data = /** @type {_tClassData} */ _extends.prototype.Class;
			class_data.parent_cdata = parent_data;

			if (__CM_ENABLE_SAFE_MODE__ && parent_data.includes_names.indexOf(class_name) != -1) t(5, class_name);
			...
			for (name in parent_data.Shared) {
				...
				if (name in class_body) {
					...
					if (__CM_ENABLE_SAFE_MODE__ && parent_group != 1 /*null*/) { // null in parent can be replaced with anything
		...
		if (('Shared' in class_body) || ('Merged' in class_body)) {
			...
			if ('Shared' in class_body) {
				...
				if (__CM_ENABLE_SAFE_MODE__ && !Array.isArray(class_body.Shared)) t(9, 'Shared');
		...
		if ('Implements' in class_body) {
			...
			if (__CM_ENABLE_SAFE_MODE__ && !Array.isArray(_implements)) t(9, 'Implements');
			for (i = 0, count = _implements.length; i < count; i++) {

				if (__CM_ENABLE_SAFE_MODE__ && !this.isValidClass(_implements[i])) t(28, i);
	...
	instanceOf: function(instance, constructor) {
		...
		if (__CM_ENABLE_SAFE_MODE__ && !this.isValidClass(constructor)) t();
